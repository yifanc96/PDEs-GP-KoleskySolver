...
function ldiv!(x, precond::precond_Theta_train, b)
    x[precond.P] = precond.U*(precond.U'*b[precond.P])
end
...
@time sol = iterGPR_fast_pcg(eqn, cov, X_domain, X_boundary, sol_init, nugget, GNsteps; ρ_big = ρ_big, ρ_small = ρ_small, k_neighbors=k_neighbors);
